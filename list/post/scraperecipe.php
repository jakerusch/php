...
if($count!=0) {
  ...
  if(!is_null($elements)) {
    foreach($elements as $element) {
      ...
      foreach($nodes as $node) {
        // insert title
        $sql = "INSERT INTO master_recipes(recipe_id, user_id, recipe_name, recipe_location)
          VALUES('$uid','$sid','".mysqli_real_escape_string($conn, $node->nodeValue)."','".mysqli_real_escape_string($conn, $url)."')";
  ...
  if(!is_null($elements)) {
    foreach($elements as $element) {
      ...
      foreach($nodes as $node) {
        // insert ingredients
        $sql = "INSERT INTO recipe_ingredients(unique_id, ingredient_order, ingredient_text)
          VALUES('$unique_id','$count','".mysqli_real_escape_string($conn, $node->nodeValue)."')";
        $count=$count+1;
        if ($conn->query($sql) === TRUE) {
        } else {
          $success="";
  ...
  if(!is_null($elements)) {
    foreach($elements as $element) {
      ...
      foreach($nodes as $node) {
        // insert directions
        $sql = "INSERT INTO recipe_directions(unique_id, directions_order, directions_text)
          VALUES('$unique_id','$count','".mysqli_real_escape_string($conn, $node->nodeValue)."')";
        $count=$count+1;
        if ($conn->query($sql) === TRUE) {
        } else {
